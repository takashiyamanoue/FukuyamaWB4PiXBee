...
import org.yamaLab.XBeeConnector.XBee;
...
//  		  pi4j=null;  // for debug
  		...
  		if(xbee==null){
  		    xbee = new XBee(this);   // uncomment for raspberry pi
//  		  pi4j=null;  // for debug
  		   this.putApplicationTable("xbee", xbee);
  		}
	...
	public void setSetting(Properties s){
		this.setting=s;
		twitterController.setSetting(s);
		pycon.setSetting(s);
	}

	public void setGui(WikiBotV1Gui g){
		this.twitterController.setGui(g.getTabbedPane());		
		this.pycon.setGui(g.getTabbedPane());
		this.pi4j.setGui(g.getTabbedPane());
		this.xbee.setGui(g.getTabbedPane());
	}
	...
	public boolean parseCommand(String cmd, String v) {
		...
		if(cmd.startsWith("tweet ")){
			...
			if(Util.parseKeyWord(l, "when ", rest)) {
				...
				twitterController.tweetWhen(param1[0],param2[0]);
		...
		if(cmd.startsWith("twitterLogin")){
